...
public class TicTacToe {
	public static void main(String[] args) {
		/* The reason we have the Scanner outside the run method is because we can't close it after
		 * every run() call. This is caused by the fact that the close() method also actually
		 * closes the System.in stream as well as the Scanner stream. And, from what I can tell,
		 * you can't re-open the System.in stream after it has been closed.
		 */
		
		Scanner input = new Scanner(System.in);
		
		// Run game as many times as user wants
		while (run(input));
	...
	// Returns true/false based on if player wants to play again
	private static boolean run(Scanner input) {
